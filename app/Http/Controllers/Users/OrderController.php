...
class OrderController extends Controller {
    ...
    public function cartStore() {
        $express_id = intval(request('express_id'));
        $spec = unserialize(request('spec'));
        $res = OrderService::cartStore($express_id, $spec);
        return response()->json($res);
    }
    ...
    private function wechatUnity($res) {
        ...
        $result = $app->order->unify([
            ...
            'openid' => isWeixin() ? session('openid') : session('user')->openid,
