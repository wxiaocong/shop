...
class OrderDao extends BaseDao
{
    ...
    {
        $res = Order::where(array('order_sn' => $orderSn, 'user_id' => session('user')->id, 'state' => 1))->update(array('state' => 6));
        if($res) {
            //活动需要释放预下单数
            DB::update("UPDATE `promotions` p, `order_goods` g, `order` o SET p.order_num = p.`order_num` - g.`num` WHERE p.id = g.`promotions_id` AND g.`order_id` = o.`id` AND o.`order_sn` = '{$orderSn}'");
        }
        return $res;
    }
